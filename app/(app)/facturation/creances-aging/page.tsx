      ...
      <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-xl font-medium tabular-nums">{formatCurrency(totals.totalDu)}</p>
          </CardContent>
        </Card>
        {buckets.map((b) => (
          <Card key={b.label}>
            <CardContent className="p-4">
              ...
              <p className={`text-xl font-medium tabular-nums ${b.tone}`}>{formatCurrency(b.value)}</p>
            </CardContent>
          </Card>
        ))}
      </div>
          ...
          {clients.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {clients.map((c) => (
                    <tr key={c.clientId} className="safe-zoom-rang border-b border-si-line ">
                      ...
                      <td className="py-2.5 px-4 text-right tabular-nums font-medium">{formatCurrency(c.totalDu)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
