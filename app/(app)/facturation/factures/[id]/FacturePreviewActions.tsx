        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setShowSend(false)}>
          <div className="w-full max-w-md rounded-2xl bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-neutral-200 px-5 py-3">
              <h3 className="text-[15px] font-semibold text-neutral-900">{t("sendInvoiceTitle")}</h3>
              <button type="button" onClick={() => setShowSend(false)} className="text-neutral-400 hover:text-neutral-700" aria-label="Fermer">
                <X className="h-4 w-4" />
              </button>
            </div>
            <div className="px-5 py-4">
              <p className="text-sm text-neutral-600">{t("sendInvoiceBody")}</p>
              <div className="mt-3">
                <div className="mb-1.5 flex items-center gap-1.5 text-[12px] font-medium text-neutral-600">
                ...
                {loadingDocs ? (
                  ...
                  <p className="py-2 text-xs text-neutral-400">{t("noAttachableDocs")}</p>
                ) : (
                  <ul className="max-h-48 space-y-1 overflow-y-auto">
                    {docs.map((d) => (
                      <li key={d.id}>
                        <label className="flex cursor-pointer items-center gap-2 rounded-md px-2 py-1.5 text-sm hover:bg-neutral-50">
                          <input type="checkbox" checked={selected.has(d.id)} onChange={() => toggle(d.id)} className="accent-forest-700" />
                          <span className="flex-1 truncate text-neutral-800">{d.titre}</span>
                          {d.statut === "brouillon" ? (
              ...
              <div className="mt-4 flex items-center justify-end gap-2">
                <button type="button" onClick={() => setShowSend(false)} className="rounded-lg border border-neutral-200 px-4 py-2 text-sm font-medium text-neutral-600">
                  {t("cancel")}
                </button>
