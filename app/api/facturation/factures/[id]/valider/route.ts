...
import { approveInvoice } from "@/lib/services/billing";
...
export async function POST(
  ...
  if (invoice.invoiceStatus !== "DRAFT") {
    return NextResponse.json(
      { error: "Seule une facture brouillon peut être approuvée" },
      { status: 400 }
    );
  }
  ...
  try {
    await approveInvoice({ invoiceId: id, approvedById: userId, cabinetId });
