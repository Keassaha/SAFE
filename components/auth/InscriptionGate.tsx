...
export function InscriptionGate() {
  ...
  function handleLanguageSelect(locale: "fr" | "en") {
    ...
    setShow(false);
    setTimeout(() => router.push("/audit-gratuit"), 500);
  }
  ...
  return (
    <div className="min-h-screen auth-container flex items-center justify-center px-4 py-10">
      {step === 1 && (
        <div className={`w-full max-w-lg transition-all duration-500 ease-out ${vis}`}>
      ...
      {step === 2 && (
        <div className={`w-full max-w-md text-center transition-all duration-500 ease-out ${vis}`}>
          <div className="mb-10 flex justify-center">
            <SafeLogo variant="dark" className="shrink-0" />
          </div>
          <p className="mb-10 text-lg font-medium text-white/85">{t("chooseLanguage")}</p>
