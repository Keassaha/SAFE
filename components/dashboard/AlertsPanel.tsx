  ...
  if (alerts.length === 0) {
    return (
      <div className="bg-white rounded-2xl border border-[#d0ddd6] shadow-sm p-5 md:p-6">
        <h3 className="text-base font-semibold text-[#1a2e28] mb-3 flex items-center gap-2">
        ...
        <div className="flex flex-col items-center py-6 text-center">
          ...
          <p className="text-sm font-medium text-[#1a2e28]">{t("noAlerts")}</p>
          <p className="text-xs text-[#6b8f7b] mt-0.5">Tout est en ordre</p>
        </div>
      </div>
    );
  }
      ...
      <ul className="space-y-2" role="list">
        {alerts.map((alert, index) => (
          <li key={`${alert.type}-${index}`}>
            <Link
              href={alert.href}
              className="flex items-center gap-3 p-3 rounded-xl bg-amber-50 border border-amber-100 hover:bg-amber-100 hover:border-amber-200 transition-all group"
            >
              <AlertTriangle className="w-4 h-4 shrink-0 text-amber-600" aria-hidden />
              <div className="min-w-0 flex-1">
                <p className="text-sm font-medium text-[#1a2e28]">{alert.type}</p>
