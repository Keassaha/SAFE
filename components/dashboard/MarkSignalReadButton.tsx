...
export function MarkSignalReadButton({ signalId }: MarkSignalReadButtonProps) {
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    startTransition(async () => {
      ...
      if (!res.ok) {
        if (res.error === "forbidden") toast.error(tUi("signalNotForYou"));
        else toast.error(tUi("actionImpossible"));
        return;
      }
      if (res.alreadyRead) {
        toast.info(tUi("alreadyMarkedRead"));
      } else {
        toast.success(tUi("markedRead"));
      }
      router.refresh();
    });
  };
