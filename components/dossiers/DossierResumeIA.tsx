...
export function DossierResumeIA({ dossierId, initialResume, canSave }: DossierResumeIAProps) {
  ...
  return (
    <section className="px-6 py-5 border-b border-si-line bg-si-surface">
      ...
      {error && <p className="text-sm text-[#B84A3E] mb-3">{error}</p>}

      {!summary && initialResume && (
        <div className="rounded-md border border-si-line bg-si-canvas p-3 text-sm text-si-ink whitespace-pre-wrap">
          {initialResume}
        </div>
      )}

      {summary && (
        <div className="rounded-md border border-si-line bg-si-surface p-4 space-y-4">
          <div className="flex items-start gap-2 rounded-md bg-si-amber/[0.13] border border-si-amber/30 px-3 py-2 text-xs text-si-amber-ink">
          ...
          {summary.synthese && <p className="text-sm text-si-ink">{summary.synthese}</p>}
          ...
          {canSave && (
            <div className="flex items-center gap-3 pt-1">
              ...
              {saved && <span className="text-xs text-si-verified">Enregistré ✓</span>}
            </div>
          )}
        </div>
      )}
