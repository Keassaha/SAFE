  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        {showAdd && (
          <form action={createDossierTache} className="rounded-lg border border-si-line p-4 space-y-3 bg-si-surface/30">
            <input type="hidden" name="dossierId" value={dossierId} />
            <Input label={t("eventTitle")} name="titre" required placeholder={t("taskTitle")} />
            <div>
              <label className="block text-sm font-medium text-si-muted mb-1">{tc("description")}</label>
              <textarea
                name="description"
                rows={2}
        ...
        {tasks.length === 0 && !showAdd ? (
          <p className="text-sm text-si-muted">{t("noTasks")}</p>
        ) : (
              ...
              editingId === task.id ? (
                <li key={task.id}>
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-3">
                      ...
                      <div>
                        <label className="block text-sm font-medium text-si-muted mb-1">{t("deadline")}</label>
                        <Input
                          name="dateEcheance"
                          type="date"
                          defaultValue={
                ...
                >
                  <div className="min-w-0 flex-1">
                    <p className="font-medium text-si-ink truncate">{task.titre}</p>
