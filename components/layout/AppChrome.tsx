...
export function AppChrome({ children, role, user, cabinetId, billingMode, activeNavIds, hiddenNavIds, trustStatus }: AppChromeProps) {
  const [mobileNavOpen, setMobileNavOpen] = useState(false);

  return (
    <div className="flex h-[100dvh] bg-[var(--safe-neutral-page)] font-sans overflow-hidden text-[var(--safe-text-title)]">
      <Sidebar role={role} billingMode={billingMode} activeNavIds={activeNavIds} hiddenNavIds={hiddenNavIds} />
      <MobileSidebar
        open={mobileNavOpen}
      ...
      <div className="flex flex-1 flex-col min-w-0 min-h-0 overflow-hidden">
        ...
        <main
          className="flex-1 px-3 sm:px-4 md:px-8 py-4 sm:py-6 overflow-y-auto flex flex-col relative overscroll-contain"
          role="main"
          style={{
            background:
