...
| **M-5** | **Export mappable QuickBooks/Xero/Sage** | Seul un CSV/Excel **générique** du journal existe. Pas de mappage de comptes, pas d'export par période verrouillé et horodaté, pas de format cible par logiciel. | §1, §2 doctrine — export externe |
| **M-6** | **Profil comptable du cabinet + onboarding** | Aucun champ de profil sur `Cabinet` (fidéicommis actif/inactif, méthode de facturation, fréquence taxes, comptable externe, logiciel). Les enums `TailleCabinet`/`ModeFacturationCabinet` existent mais sur `Lead`, pas sur `Cabinet`. Aucun questionnaire d'onboarding comptable. | §10 doctrine + SOP — adaptation au profil |
| **M-7** | **Contrôles anti-erreurs interface** | Avertissements manquants : paiement sans facture, facture sans client/dossier, débours non facturé dans un dossier fermé. (Les contrôles **moteur** existent : solde négatif, transfert sans facture, cross-client.) | doctrine §8 |
